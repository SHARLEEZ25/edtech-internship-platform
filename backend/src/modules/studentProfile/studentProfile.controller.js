...
const getStudentProfileById = async (req, res, next) => {
    try {
        const profile = await studentProfileService.getProfileByStudentId(req.params.studentId);
        successResponse(res, 200, { data: profile });
    } catch (err) {
        next(err);
    }
...
const updateBasicProfile = async (req, res, next) => {
    try {
        const result = await studentProfileService.updateBasicProfile(req.user.id, req.body);
...
const addExperience = async (req, res, next) => {
    try {
        ...
        successResponse(res, 201, { data: result });
    } catch (err) {
        next(err);
    }
};
...
const updateSkills = async (req, res, next) => {
    try {
        ...
        await studentProfileService.updateSkills(req.user.id, skills);

        successResponse(res, 200, { message: "Skills updated successfully" });
    } catch (error) {
        next(error);
    }
};
